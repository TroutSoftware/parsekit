...
func scantk(sc *parsekit.Scanner, tk rune) (rune, int) {
	...
	case tk == '{', tk == '}', tk == ';':
		return tk, 1
	case tk == '"':
		return scanstring(sc, tk)
	case '0' <= tk && tk <= '9':
		return scanumeral(sc, tk)
	default:
		return scanident(sc, tk)
	}
}

func scanstring(sc *parsekit.Scanner, lead rune) (rune, int) {
	...
	var transitions = [final][]byte{
		escaped: {'\x00', string},
		begin:   {'"', string},
		string:  {'\x00', string, '\\', escaped, '"', final},
	}

	_, n := sc.ScanWithTable(transitions[:])
	return StringToken, n + utf8.RuneLen(lead)
}

func scanident(sc *parsekit.Scanner, lead rune) (rune, int) {
	const (
		ident uint8 = iota
		final
	)
	...
	var transitions = [final][]byte{
		ident: {' ', final, '-', ident, '0', ident, '1', ident, '2', ident, '3', ident, '4', ident, '5', ident, '6', ident, '7', ident, '8', ident, '9', ident, ';', final, 'A', ident, 'B', ident, 'C', ident, 'D', ident, 'E', ident, 'F', ident, 'G', ident, 'H', ident, 'I', ident, 'J', ident, 'K', ident, 'L', ident, 'M', ident, 'N', ident, 'O', ident, 'P', ident, 'Q', ident, 'R', ident, 'S', ident, 'T', ident, 'U', ident, 'V', ident, 'W', ident, 'X', ident, 'Y', ident, 'Z', ident, '_', ident, 'a', ident, 'b', ident, 'c', ident, 'd', ident, 'e', ident, 'f', ident, 'g', ident, 'h', ident, 'i', ident, 'j', ident, 'k', ident, 'l', ident, 'm', ident, 'n', ident, 'o', ident, 'p', ident, 'q', ident, 'r', ident, 's', ident, 't', ident, 'u', ident, 'v', ident, 'w', ident, 'x', ident, 'y', ident, 'z', ident},
	}

	_, n := sc.ScanWithTable(transitions[:])
	return IdentToken, n + utf8.RuneLen(lead) - 1 // drop space or training ;
}

// unrolled state machine
// transitions generated by calling transgen.awk
// TODO(rdo) make this go:generatable
